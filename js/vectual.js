(function (window, document, undefined) {
	...
	function v(config) {
		...
		c.data.forEach(function (element, index) {
			...
			if (index == 0)
				...
				if (element.value > c.max.value)
					c.max = element;
				else if (element.value < c.min.value)
					c.min = element;
			}

			c.totalValue += Number(element.value); //get sum of all values

			c.keys.push(element.key);
			c.values.push(element.value);
			tuples.push([element.key, element.value]); //get sortable array
	...
	function Tagcloud() {
		...
		var minFontsize = 10, //minimum fontsize
	...
	// Make vectual globally available
	vectual = v;

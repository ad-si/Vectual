...
class Vectual {
	...
	for($i=0; $i<=20; $i++){
		Vectual::$color[$i] = 'rgb(255, '.rand(1,255).', '.rand(1,255).')';
	}
	...
	function __construct($v) {
		...
		$this->type = $v['type'];
		$this->type = $v['title'];
		$this->toolbar = $v['toolbar'];
		$this->automode = $v['automatic'];
		$this->doctype = $v['doctype'];
		$this->data = $v['data'];
		$this->totalValue = array_sum($sorted[1]["value"]);
		
		$this->graphWidth = ($v["width"]  - 50);
		$this->graphHeight = ($v["height"] - 100);
		
		$this->numKeys = count($v["data"][1]["key"]);
		$this->sortedValues = $v["data"][1]["value"]; 
		$this->sortedKeys = $v["data"][1]["key"];	
		array_multisort($sortedValues, $sortedKeys); //Sort by value
		$this->maxValue = max($v["data"][1]["value"]);
		$this->maxKey = array_search($this->maxValue, $v["data"][1]["value"]);
		$this->minValue = min($v["data"][1]["value"]);
		$this->minKey = array_search($this->minValue, $v["data"][1]["value"]);
	...
	public function draw() {	
		...
		return sprintf($this->svg, $this->background, $this->header, $this->toolbar, $this->compileGraph());
	}
	...
	private function compileGraph(){
			
			$new = new $this->type($this->$data);
	...
	private function compileHeader(){
		...
		$this->header .=
		'class="vectual" width="'.$this->width.'" height="'.$this->height.'" >';
	}
	
	private function compileDefs(){
	...
	private function compileBackground(){
		...
		$this->background .=
		'<text class="vectual_title" x="'.($v["width"] * 0.03).'" y="'.($v["height"] * 0.07).'" 
			style="font-height:'.($v["height"] * $v["width"] * 0.00009).'" >
				'.$v["title"].'
		</text>';
	}
	
	private function compileToolbar(){
			
		$this->toolbar .='
		<g class="toolbar_container" transform="translate('.($v["width"] - 120).', '.($v["height"] * 0.02).') scale(0.7)">';
		 
		 
			if(in_array('save', $v['toolbar'])){
				$this->toolbar .=
			...
			if(in_array('tagcloud', $v['toolbar'])){	
			 	$this->toolbar .='

        ...
        , describe "Data labels tests"
            [ test "getDataLabels extracts labels from TimeData" <|
                \_ ->
                    let
                        labels =
                            getDataLabels testConfig testTimeData
                    in
            ...
            , test "getDataLabels extracts labels from KeyData" <|
                \_ ->
                    let
                        labels =
                            getDataLabels testConfig testKeyData
            ...
            , test "getDataLabels generates labels for Values" <|
                \_ ->
                    let
                        labels =
                            getDataLabels testConfig testValues
            ...
            , test "getDataLabels returns empty list for InvalidData" <|
                \_ ->
                    let
                        labels =
                            getDataLabels testConfig InvalidData
        ...
        , describe "MetaData tests"
            [ test "getMetaData calculates correct dimensions" <|
                \_ ->
                    let
                        metaData =
                            getMetaData testConfig testTimeData
                    in
                    Expect.all
            ...
            , test "getMetaData respects yStartAtZero setting" <|
                \_ ->
                    let
                        configNoZero =
                            { testConfig | yStartAtZero = False }

                        metaData =
                            getMetaData configNoZero testTimeData
                    in
                    Expect.equal metaData.yMinimum 10.0

            -- Should use data minimum
            ]
        ]

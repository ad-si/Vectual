    ...
    describe "Vectual Data Types Tests"
        [ describe "Data type tests"
            [ test "TimeData stores time records correctly" <|
                \_ ->
                    let
                        timeRecord = { utc = millisToPosix 1000, value = 10.0, offset = 0.0 }
                        timeData = TimeData [ timeRecord ]
                    in
                    case timeData of
                        TimeData records ->
            ...
            , test "KeyData stores key records correctly" <|
                \_ ->
                    let
                        keyRecord = { key = "Apple", value = 25.0, offset = 0.0 }
                        keyData = KeyData [ keyRecord ]
                    in
                    case keyData of
                        KeyData records ->
                            case List.head records of
        ...
        , describe "Chart type tests"
            [ test "LineChart type holds config and data" <|
                \_ ->
                    let
                        config = { title = "Test", inline = False, width = 400, height = 300, borderRadius = (2, 2), xLabelFormatter = \_ -> "", labelAngle = 0, yStartAtZero = True, alignBars = Center, showAnimations = False }
                        data = Values [ 1.0, 2.0 ]
                        chart = LineChart config data
                    in
                    case chart of
            ...
            , test "BarChart type holds config and data" <|
                \_ ->
                    let
                        config = { title = "Test", inline = False, width = 400, height = 300, borderRadius = (2, 2), xLabelFormatter = \_ -> "", labelAngle = 0, yStartAtZero = True, alignBars = Center }
                        data = Values [ 1.0, 2.0 ]
                        chart = BarChart config data
                    in
                    case chart of
            ...
            , test "PieChart type holds config and data" <|
                \_ ->
                    let
                        config = { title = "Test", inline = False, width = 400, height = 300, borderRadius = (2, 2), xLabelFormatter = \_ -> "", radius = 100, showAnimations = False, yStartAtZero = True }
                        data = Values [ 1.0, 2.0 ]
                        chart = PieChart config data
                    in
                    case chart of
